...
void init()
{
    ...
    sink->locked_backend()->add_stream(
        boost::make_shared< std::ofstream >("attributes_sample.log"));

    sink->set_formatter
    (
        expr::stream
            << std::hex << std::setw(2) << std::setfill('0') << line_id << std::dec << std::setfill(' ')
            << ": <" << severity << ">\t"
            << "(" << scope << ") "
...
BOOST_AUTO_TEST_SUITE(boost_log_attribute)
